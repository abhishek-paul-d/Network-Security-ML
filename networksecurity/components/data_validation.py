...
class DataValidation:
    ...
    def validate_no_of_columns(self,dataframe: pd.DataFrame)-> bool:
        try:
            ...
            present_no_of_cols=len(dataframe.columns)
            logging.info(f"Dataframe has columns:{present_no_of_cols}")
            if number_of_cols==present_no_of_cols:
                return True
            return False
        except Exception as e :
            raise NetworkSecurityException(e,sys)
        
    def validate_numerical_features(self,dataframe:pd.DataFrame)->bool:
        try: 
            ...
            for column in expected_numerical_columns:
                if column not in dataframe_cols:
                    status=False
                    missing_numerical_features.append(column)
                if len(missing_numerical_features)>0:
                    logging.info(f"Missing numerical columns: {missing_numerical_features}")
                return status

        except Exception as e:
            raise NetworkSecurityException(e,sys)
        
    def detect_data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,threshold: float =0.05)-> None:
        try:
            status=True
            report={}
    ...
    def initiate_data_validation(self,)->DataValidationArtifact:
        try:
            ...
            status=self.validate_numerical_features(dataframe=train_dataframe)
            if not status:
                 error_message = f"Train dataframe is missing required numerical columns."
            status=self.validate_numerical_features(dataframe=test_dataframe)
            if not status:
                error_message = f"Train dataframe is missing required numerical columns."
